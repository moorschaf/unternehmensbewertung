...
with tab1:
    st.caption("Editierbar, Werte in USD Mio. (shares in Mio. shares)")
    if st.button("Reset Income auf SEC", key="reset_income_btn"):
        reset_editor_state("income_statement_editor")
    income_edit = st.data_editor(
        to_editable_millions(bundle.income_df, income_cols),
        use_container_width=True,
        key="income_statement_editor",
    )
with tab2:
    st.caption("Editierbar, Werte in USD Mio.")
    if st.button("Reset Balance auf SEC", key="reset_balance_btn"):
        reset_editor_state("balance_sheet_editor")
    balance_edit = st.data_editor(
        to_editable_millions(bundle.balance_df, balance_cols),
        use_container_width=True,
        key="balance_sheet_editor",
    )
with tab3:
    st.caption("Editierbar, Werte in USD Mio.")
    if st.button("Reset Cashflow auf SEC", key="reset_cashflow_btn"):
        reset_editor_state("cashflow_statement_editor")
    cashflow_edit = st.data_editor(
        to_editable_millions(bundle.cashflow_df, cashflow_cols),
        use_container_width=True,
        key="cashflow_statement_editor",
    )
...
hist_choice = st.selectbox(
    "Historienfenster",
    ["3 Jahre", "5 Jahre", "10 Jahre", "Alle verfuegbaren Jahre"],
    index=1,
    help="Steuert, wie viele historische Jahre fuer Umsatzwachstum und EBIT-Marge in die Basisschaetzung einfliessen.",
)
history_years = {
    "3 Jahre": 3,
    "5 Jahre": 5,
    "10 Jahre": 10,
    "Alle verfuegbaren Jahre": None,
}[hist_choice]
...
da_pct_rev_base = pct_or_default(hist_mean(driver["da_pct_rev"], 0.03), 0.03)
capex_pct_rev_base = pct_or_default(hist_mean(driver["capex_pct_rev"], 0.04), 0.04)
nwc_pct_rev_base = pct_or_default(hist_mean(driver["nwc_pct_rev"], 0.10), 0.10)
